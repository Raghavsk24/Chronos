...
export function meetingStatusConfig(status: MeetingStatus | string): { label: string; className: string } {
  switch (status) {
    case 'scheduling':
      return { label: 'Scheduling', className: 'bg-yellow-100 text-yellow-800 border-2 border-yellow-200' }
    case 'scheduled':
      return { label: 'Scheduled', className: 'bg-blue-100 text-blue-800 border-2 border-blue-200' }
    case 'completed':
      return { label: 'Completed', className: 'bg-green-100 text-green-800 border-2 border-green-200' }
    default:
